...
object Chat {
  ...
  final case class GetUsers(request:ActorRef[Session.UserList]) extends Message
    ...
    def onMessage(msg: Message): Behavior[Message] = {
      msg match {
        case SendText(login, text)   =>
          sessions.keys.foreach(_ ! Session.NewMessage(login, text))
        ...
        case Disconnect(session)     =>
          sessions -= session
        case GetUsers(request) => request ! Session.UserList(sessions.values.toList)
      }
      Actor.same
    }

...
class SQLAPI(resource: String) {
  ...
  def execute(sql: String): String = logParameter((connection andThen openConnection)(logParameter(resource))(logParameter(sql)))


  private def logParameter[T](prm: T): T  = {println(prm.toString); prm}

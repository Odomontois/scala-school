...
object SortingStuff extends App {
  ...
  private def sort(stuff: List[Stuff], stuffBox: StuffBox): StuffBox = stuff match {
    ...
    case item :: rest => val newBox = putStuffInRightBox(item, stuffBox)
      sort(rest, newBox)
  }
  ...
  private def putStuffInRightBox(item: Stuff, stuffBox: StuffBox): StuffBox =
    item match {
      ...
      case it: Boots if it.brand.toLowerCase == "adidas" || it.brand.toLowerCase == "converse" =>
  ...
  def findMyKnife(stuffBox: StuffBox): Boolean = stuffBox match {
    case sb: StuffBox if sb.junk.contains(Knife) => true
    case _ => false
  }
